...
public class SignupReq {
    @Schema(description="실명(문자열): 필수", required = true, example = "gildong")
    @NotBlank
    @Pattern(regexp = "[0-9A-Za-z가-힣]+", message="signup wrong name")
    private String name;
    @Schema(description="이메일(문자열): 필수, 로그인 아이디임", required = true, example = "example@example.com")
    @NotBlank
    @Email(message = "signup wrong email")
    private String email;
    @Schema(description="패스워드(문자열): 8자 이상 영문, 숫자, 일부 특수문자 사용, 이건 평문이며, 서버 내에서 암호화",required = true )
    @NotBlank
