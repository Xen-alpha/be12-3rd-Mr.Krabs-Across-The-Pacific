...
메인 백엔드는 아마존 웹 서비스의 EC2 서비스에 올라가 있으며, 메인 백엔드는 사용자, 포트폴리오 관리, 주식 종목 수 관리를 담당한다.

[크롤링 서버](https://github.com/Xen-alpha/polygon.io-crawltest)는 MongoDB에 나스닥의 모든 종목의 종가를 크롤링하여 MongoDB Atlas 서비스를 통해 메인 백엔드에 데이터를 전달한다.

두 개의 클라우드로 분리한 이유는 크롤링 서버가 있는 오라클 클라우드의 프리 티어 정책이 무료 VM과 10TB의 아웃바운드 트래픽을 무료로 제공하기에 크롤링 및 주가 데이터 전송에 필요한 대용량 트래픽을 감당하기 적절하다고 판단하였기 때문이다.
